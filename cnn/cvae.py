...
xtr = torch.randn(10,4,64,64,64)
ytr = torch.randn(10,64,64,31)
...
class Encoder(nn.Module):
    ...
    def forward(self, x):   
        for idx in range(len(self.layers)):
            x = self.layers[idx](x)
        mu = nn.Linear(x,latent_dim)
        log_var = nn.Linear(x,latent_dim)
        z = sampling()([mu0,log_var0])
        return  x, mu,log_var

class cvae(nn.Module):
    def __init__(self):
        super(cvae, self).__init__()
        self.encoder0 = Encoder(1,16)
        self.encoder1 = Encoder(1,16)
        self.encoder2 = Encoder(1,16)
        self.encoder3 = Encoder(1,16)
        ...
        self.decoder.append(sub_seq)
    def forward(self, x):
        x0,mu0,log_var0 = self.encoder0(x[:, 0:1, :, :, :])
        x1,mu1,log_var1 = self.encoder1(x[:, 1:2, :, :, :])
        x2,mu2,log_var2 = self.encoder2(x[:, 2:3, :, :, :])
        x3,mu3,log_var3 = self.encoder3(x[:, 3:4, :, :, :])
        # one-hot encoding
        condition = np.eye(31)[class_label]
        z = torch.cat([z0,z1,z2,z3,condition],dim=-1)
        ...
        return x,mu0,log_var0,mu1,log_var1,mu2,log_var2,mu3,log_var3
...
def loss_func(target,pred):
    reconstruction_loss = tf.reduce_mean(
                keras.losses.binary_crossentropy(x[:, :-self.condition_dim], x_prime))
    # KL divergence
